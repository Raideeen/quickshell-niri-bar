...
Item {
  ...
  implicitWidth: {
    ...
    if (quickIndicator.visible) {
      w += quickIndicator.implicitWidth;
      visibleCount++;
    }
    if (graph.visible) {
      w += graph.width + graphTextMargin;
      visibleCount++;
    }
  ...
  RowLayout {
    ...
    Text {
      ...
      text: {
        let cpuLoadAvgOneMin = CPU.loadAvg[0] !== undefined ? CPU.loadAvg[0].toFixed(2) : 0;
    ...
    Canvas {
      ...
      onPaint: {
        ...
        let visiblePoints = points.filter(function (p) {
            return p.x >= minTime;
        });
