...
Item {
    ...
    implicitWidth: {
        let w = 0;
        let visibleCount = 0;
        if (icon.visible) { w += icon.width; visibleCount++; }
        if (quickIndicator.visible) { w += quickIndicator.implicitWidth; visibleCount++; }
        if (graph.visible) { w += graph.width + graphTextMargin; visibleCount++; }
        // account for RowLayout spacing between visible items
        if (visibleCount > 1) w += (visibleCount - 1) * iconText.spacing;
        return w;
    }
    ...
    RowLayout {
      ...
      Text {
        id: quickIndicator
        visible: Config.cpuQuickIndicatorEnabled
        text: {
          let cpuLoadAvgOneMin = CPU.loadAvg[0].toFixed(2);
          return `${cpuLoadAvgOneMin}/100%`
        }
        ...
        contentComponent: Component {
            ColumnLayout {
                ...
                Text {
                    ...
                    text: {
                        let out = [];
                        let height = Config.cpuCoreCanvasHeight;
                        let width = Math.ceil(CPU.coreUsages.length / Config.cpuCoreCanvasHeight);

                        // column-major indexing
                        // A[y][x] so columns is like rows and rows are like columns
                        for (let y = 0; y < height ?? 1; ++y) {
                          for (let x = 0; x < width; ++x) {
                            let index = y+height*x;
                            let coreNum = `Core ${index+1}:`.padEnd(10);
                            if (index < CPU.coreUsages.length) {
                              let coreUsage = CPU.coreUsages[index].toFixed(2);
                              x == 0 ? out.push(`\n${coreNum}${coreUsage}`) : out.push(`${coreNum}${coreUsage} `)
                            }
                          }
                        }
