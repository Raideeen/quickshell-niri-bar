...
Singleton {
    ...
    Process {
        id: psProc
        command: ["sh", "-c", `ps -eo pid,comm,%mem --sort=-%mem --no-headers | head -${Config.ramNumTopProcesses+10}`]
        stdout: StdioCollector {
            onStreamFinished: {
                let lines = this.text.split('\n').filter(l => l.trim() !== '');
                let tp = [];
                for (let line of lines) {
                    if (tp.length == Config.ramNumTopProcesses) break;
    ...
    Timer {
        interval: Config.ramUpdateInterval
        running: true
        repeat: true
